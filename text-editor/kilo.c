...
void editorUpdateRow(erow *row) {
	...
	free(row->render);
	row->render = malloc(row->size + tabs * (KILO_TAB_STOP - 1) + 1); //mallocing enough tab room for 8 spaces: row->size alr contains 1 space
...
void editorInsertChar(char c) {
	if (E.cy == E.numrows) { // if at end of document append line
...
char* editorRowsToString(int *buflen){
	...
	*buflen = totlen;
	...
	for (j = 0; j < E.numrows; j++) {
		...
		p += E.row[j].size; //cheecky pointer arithmetic
		*p = '\n';
		p++;
	}
	return buf;
...
void editorSave(){
	...
	//open for reading and writing (O_RDWR), create if doesnt exist (O_CREAT), standard text-file permissions (0644)
	int fd = open(E.filename, O_RDWR | O_CREAT, 0644); 
	if (fd != -1) {
		if (ftruncate(fd, len) != -1) {//set length of file
			if (write(fd, buf, len) == len){
				close(fd); //everything succeeded
				free(buf);
				E.dirty = 0;
...
void editorDrawStatusBar(struct abuf *ab) {
	abAppend(ab, "\x1b[7m", 4); //Select Graphic Rendition: inverts colors
	...
	abAppend(ab, "\x1b[m", 3); //reverts colors
	abAppend(ab, "\r\n", 2);
...
void editorDrawMessageBar(struct abuf *ab){
	...
	if (E.screencols < msglen) msglen = E.screencols;
	if (msglen && time(NULL) - E.statusmsg_time < 5) { // if under 5 seconds have passed
...
void editorSetStatusMessage(const char *fmt, ...){ //vardic function
	va_list ap;
	va_start(ap, fmt);
	vsnprintf(E.statusmsg, sizeof(E.statusmsg), fmt, ap);
	va_end(ap);
	E.statusmsg_time = time(NULL); //gets current time
}
...
void initEditor() {
	...
	E.screenrows -= 2; //room for status bar
}
